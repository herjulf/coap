...
void usage(void)
{
  ...
  printf("  * A simple CoAP pubsub subscriber\n");
...
int process(void)
{
    ...
    if(subscribe_uri) {
      ...
      if(send_len) {
	if(debug & D_COAP_PKT)
	  dump_pkt((struct coap_hdr*)buf, send_len);
